...
class upload_fw(Task.Task):
    ...
    def run(self):
        ...
        if 'AP_OVERRIDE_UPLOAD_CMD' in os.environ:
            ...
            # As of Feb 2022 WSL2 does not support native USB support. The workaround from Microsoft using 'usbipd' does not work due to the following workflow:
            # 1) connect USB device to Windows compuiter running WSL2
            ...
            # 5) uploader.py detects device, sends reboot command which disconnects the USB port and reboots into bootloader (different USB device)
